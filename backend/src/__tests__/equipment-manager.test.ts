...
describe("getUniqueName", () => {
	...
	it("incrementa além do maior número existente", () => {
		expect(
			getUniqueName("Scarlet", ["Scarlet", "Scarlet 1", "Scarlet 2"]),
		).toBe("Scarlet 3");
	});
...
describe("cloneSet", () => {
	...
	it("copia os dados de equipment do original", () => {
		...
		const clone = cloneSet(original, ["Armor"]);
		expect(clone.equipment.capacete).toEqual({
			has_in_set: true,
			acquired: true,
			custom_name: "",
		});
...
describe("updateEquipmentAcquisition", () => {
	...
	it("lança erro quando o item não existe no set", () => {
		...
		expect(() =>
			updateEquipmentAcquisition(set, "inexistente", true),
		).toThrow();
	});
...
describe("bulkUpdateSets", () => {
	...
	it("atualiza has_in_set de um item de equipment existente", () => {
		const sets = [
			makeSet({ id: "a", equipment: { capacete: makeItem(false, false) } }),
		];
		const result = bulkUpdateSets(sets, [{ id: "a", capacete: true }]);
		expect(
			result.find((s) => s.id === "a")?.equipment.capacete.has_in_set,
		).toBe(true);
	});
