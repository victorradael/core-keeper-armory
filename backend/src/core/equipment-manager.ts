...
import type { EquipmentSet, NewEquipmentSet, PreparedEquipmentSet } from '../types';

/**
 * Prepara um conjunto para exibição.
 */
export function prepareSetForDisplay(set: EquipmentSet): PreparedEquipmentSet {
  const itemsInSet = Object.values(set.equipment).filter(item => item.has_in_set);
...
export function getUniqueName(baseName: string, existingNames: string[]): string {
  ...
  for (const name of existingNames) {
    if (name === prefix) {
      ...
      const nameMatch = name.match(new RegExp(`^${prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} (\\d+)$`));
      if (nameMatch) {
 ...
 */
export function createSet(data: NewEquipmentSet, existingNames: string[]): EquipmentSet {
  return {
    ...data,
    id: uuidv4(),
 ...
 */
export function cloneSet(original: EquipmentSet, existingNames: string[]): EquipmentSet {
