...
function SessionBadge({ code }: { code: string }) {
	...
	return (
		<div className="flex items-center gap-4 mb-3 px-5 py-3 border-2 border-white/10 bg-black/20">
			<span className="text-sm font-pixel text-[#938F99] uppercase tracking-widest">
				SESSION
			</span>
			<span className="text-xl font-pixel text-primary font-black tracking-widest">
				{code}
			</span>
			<button
				type="button"
...
export function TabBar({ activeTab, onTabChange }: TabBarProps) {
	...
	return (
		<div className="flex items-end justify-between bg-black/40 border-b-4 border-white/5 px-4 pt-4">
			...
			{config?.app_code && <SessionBadge code={config.app_code} />}
		</div>
	);
}
