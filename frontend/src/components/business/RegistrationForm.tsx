...
import { 
  HardHat, Shirt, Footprints, Circle, Gem, Backpack, 
  ShieldAlert, ShieldCheck, User, Hammer, Box 
} from 'lucide-react';
...
export function RegistrationForm({ catalog = {}, onSuccess }: RegistrationFormProps) {
  ...
  // Atualizar default values quando o catálogo carregar
  useEffect(() => {
    if (Object.keys(catalog).length > 0) {
      reset({
        name: '',
        type: 'R',
        equipment: Object.keys(catalog).reduce((acc, key) => ({
          ...acc,
          [key]: { has_in_set: false, acquired: false, custom_name: '' }
        }), {})
      });
    }
  }, [catalog, reset]);
  ...
  if (Object.keys(catalog).length === 0) {
    return <div className="text-center py-12 font-pixel text-[#938F99]">LOADING BLUEPRINT DATA...</div>;
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        <div className="pixel-card p-8 space-y-8 border-primary/20">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="font-pixel text-xs text-primary uppercase tracking-widest ml-1">Set Designation</label>
              <input 
              ...
              />
              {errors.name && <p className="text-[10px] font-pixel text-red-500 uppercase">{errors.name.message as string}</p>}
            </div>
            
            <div className="space-y-2">
              <label className="font-pixel text-xs text-primary uppercase tracking-widest ml-1">Owner</label>
                ...
                {[
                  ...
                  <label key={owner.value} className="flex-1">
                    ...
                    <div className={cn(
                      ...
                      "peer-checked:" + owner.color + " peer-checked:text-white",
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-xl font-black text-white font-pixel uppercase tracking-widest">COMPONENT SELECTION</h3>
              ...
              return (
                <div key={key} className={cn(
                  ...
                  <div className="flex items-center gap-3">
                    <Checkbox 
                      {...register(`equipment.${key}.has_in_set`)}
                    />
                    <div className="flex items-center gap-2">
                      <Icon className={cn("w-4 h-4", isSelected ? "text-primary" : "text-[#938F99]")} />
                      <span className={cn(
                  ...
                  <input 
                    ...
                    className={cn(
                      ...
                      isSelected ? "border-primary/20 focus:border-primary" : "border-white/5 opacity-50"
                    )}
                  />
                </div>
              );
