...
import { 
  ChevronDown, ChevronUp, Copy, Trash2, Trophy, Pickaxe, 
  User, HardHat, Shirt, Footprints, Circle, Gem, Backpack, ShieldAlert,
  ShieldCheck, Box
} from 'lucide-react';
import { Card } from '../ui/Card';
import { Checkbox } from '../ui/Checkbox';
import { Button } from '../ui/Button';
import { useUpdateItem, useCloneSet, useDeleteSet } from '../../hooks/useSets';
      ...
      <div className="flex flex-col gap-4 mt-2">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={cn(
              "p-2 border-2",
              set._is_complete ? "border-secondary text-secondary" : "border-white/10 text-[#938F99]"
            )}>
              {set._is_complete ? <Trophy className="w-6 h-6" /> : <Pickaxe className="w-6 h-6" />}
            </div>
            <div>
              <h3 className="text-xl font-bold font-pixel text-white leading-tight">{set.name}</h3>
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
             <Button variant="text" size="sm" onClick={() => cloneSet.mutate(set.id)}>
              <Copy className="w-4 h-4 text-[#938F99]" />
            </Button>
            <Button variant="text" size="sm" onClick={() => confirm(`Apagar ${set.name}?`) && deleteSet.mutate(set.id)}>
          ...
          <div className="pt-2 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
            {Object.entries(set.equipment)
              ...
              .map(([key, data]) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-3 p-2 bg-black/20 border border-white/5">
                    <Checkbox 
                      checked={data.acquired}
                      onChange={(e) => updateItem.mutate({ setId: set.id, key, acquired: e.target.checked })}
                    />
                    <div className="flex flex-col flex-1 min-w-0">
                      <span className={cn(
                        "text-xs font-bold truncate",
                        data.acquired ? "text-white/40 line-through" : "text-white"
                      )}>
