		...
		const flatData: TableRow[] = sets.map((set) => {
			const equipmentFields = Object.fromEntries(
				Object.keys(catalog).map((key) => [
					key,
					set.equipment?.[key]?.has_in_set || false,
				]),
			);
			return {
				id: set.id,
				...
				<table className="w-full text-left border-collapse">
					<thead>
						<tr className="bg-white/5 border-b-2 border-white/10">
							<th className="p-4 text-xs font-black text-primary uppercase tracking-[0.2em] font-pixel">
								SET DESIGNATION
							</th>
							<th className="p-4 text-xs font-black text-primary uppercase tracking-[0.2em] font-pixel text-center">
								OWNER
							</th>
							{equipmentKeys.map((key) => {
								...
								return (
									<th
										key={key}
										className="p-4 text-xs font-black text-primary uppercase tracking-[0.2em] font-pixel text-center"
									>
										<div className="flex flex-col items-center gap-1">
											<Icon className="w-4 h-4" />
											<span className="text-[8px]">
												{catalog[key]?.toUpperCase() || key}
											</span>
										</div>
									</th>
								);
							...
							>
								<td className="p-2 min-w-[200px]">
									<input
										...
										value={row.Nome}
										onChange={(e) =>
											handleInputChange(row.id, "Nome", e.target.value)
										}
									...
									<td key={key} className="p-2 text-center">
										<label className="inline-flex items-center justify-center cursor-pointer p-3 rounded-lg hover:bg-white/5 transition-all">
											<input
												...
												checked={!!row[key]}
												onChange={(e) =>
													handleInputChange(row.id, key, e.target.checked)
												}
